package com.program.training.systemdesign.businesssystemsimulation;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

/**
 * Q2. Design Twitter
 *
 * @author naletov
 */
class TwitterTest
{
    @Test
    void twitterTest()
    {
        Twitter twitter = new Twitter();

        twitter.postTweet(1, 5); // User 1 posts a new tweet (id = 5).
        assertEquals("[5]", twitter.getNewsFeed(1).toString(),
                "User 1's news feed should return a list with 1 tweet id -> [5]. return [5]");

        twitter.follow(1, 2);    // User 1 follows user 2.
        twitter.postTweet(2, 6); // User 2 posts a new tweet (id = 6).

        assertEquals("[6, 5]", twitter.getNewsFeed(1).toString(),
                " User 1's news feed should return a list with 2 tweet ids -> [6, 5]. Tweet id 6 should precede tweet id 5 because it is posted after tweet id 5.");

        twitter.unfollow(1, 2);  // User 1 unfollows user 2.
        assertEquals("[5]", twitter.getNewsFeed(1).toString(),
                "User 1's news feed should return a list with 1 tweet id -> [5], since user 1 is no longer following user 2");
    }

    @Test
    void postTweetPostTweetGetNewsFeed()
    {
        Twitter twitter = new Twitter();

        twitter.postTweet(1, 5);
        twitter.postTweet(1, 3);

        assertEquals("[3, 5]", twitter.getNewsFeed(1).toString(),
                "User 1's news feed should return [3, 5], tweet 3 is more recent than tweet 5");
    }
}